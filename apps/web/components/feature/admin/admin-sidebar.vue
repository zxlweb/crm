<template>
  <aside class="hidden w-64 shrink-0 flex-col border-r border-ds-border bg-ds-bg-sidebar lg:flex">
    <div class="flex h-16 items-center gap-2.5 border-b border-ds-border-muted px-6">
      ...
      <div>
        <p class="text-sm font-bold text-ds-fg-heading">{{ brandName }}</p>
        <p class="text-[10px] font-medium uppercase tracking-wider text-ds-fg-subtle">Super Admin</p>
      </div>
    </div>
    ...
    <nav class="flex-1 space-y-1 p-4">
      <NuxtLink
        ...
        :class="isActive('/admin') ? 'ds-nav-active' : 'text-ds-fg-nav hover:bg-ds-bg-muted hover:text-ds-fg-nav-active'"
      >
      ...
      <button
        ...
        class="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-ds-fg-nav transition-colors hover:bg-ds-bg-muted hover:text-ds-fg-nav-active"
    ...
    <div class="space-y-2 border-t border-ds-border-muted p-4">
      <div class="px-1">
        <UiThemeToggle />
      </div>
      <NuxtLink
