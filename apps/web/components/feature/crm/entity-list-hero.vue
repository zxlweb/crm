<template>
  <section
    class="ds-entity-list-hero relative overflow-hidden rounded-2xl border border-ds-border bg-ds-bg-elevated/85 p-4 shadow-ds-sm backdrop-blur-sm sm:p-5"
    :data-testid="testId"
  >
    <span
    ...
    <div class="relative grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <article
        ...
        :key="stat.key"
        class="ds-entity-list-hero__tile group relative overflow-hidden rounded-xl border border-ds-border-muted bg-ds-bg-elevated/70 p-3.5 transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:border-ds-brand-muted/60 hover:shadow-ds-md"
        :class="stat.featured ? 'ds-entity-list-hero__tile--featured' : ''"
      >
        <span
...
export type EntityListHeroItem = {
...
const props = withDefaults(
  defineProps<{
    items: EntityListHeroItem[]
    total: number
    /** i18n prefix, e.g. `accountsHero` → accountsHeroStatTotal */
    i18nPrefix: string
    atRiskHref?: string
    testId?: string
  }>(),
  {
    atRiskHref: undefined,
    testId: 'entity-list-hero',
  },
)
...
function tk(suffix: string, params?: Record<string, number | string>) {
  return params ? t(`${props.i18nPrefix}${suffix}`, params) : t(`${props.i18nPrefix}${suffix}`)
}

const hotCount = computed(
  () => props.items.filter((row) => row.engagement_score >= HOT_THRESHOLD).length,
)

const atRiskCount = computed(
  () => props.items.filter((row) => row.relationship_health === 'low').length,
)
...
const idleCount = computed(() => {
  ...
  return props.items.filter((row) => {
    ...
    const diff = Math.floor((now - new Date(row.last_activity_at).getTime()) / 86_400_000)
...
const stats = computed<HeroStat[]>(() => [
  {
    ...
    label: tk('StatTotal'),
    value: props.total,
    hint: tk('StatTotalHint', { n: props.items.length }),
  ...
  },
  {
    ...
    tagLabel: hotCount.value > 0 ? tk('StatHotTag') : tk('StatQuietTag'),
  ...
  },
  {
    ...
    tagLabel: atRiskCount.value > 0 ? tk('StatActNow') : tk('StatHealthy'),
    tagIcon:
      atRiskCount.value > 0
    ...
    tagClass:
      atRiskCount.value > 0
        ...
        : 'bg-ds-success-subtle text-ds-success ring-ds-success/25',
    href: atRiskCount.value > 0 ? props.atRiskHref : undefined,
  },
  {
    key: 'idle',
    label: tk('StatIdle'),
    value: idleCount.value,
    hint: tk('StatIdleHint', { days: IDLE_THRESHOLD_DAYS }),
    ...
    tagLabel: idleCount.value > 0 ? tk('StatFollowUp') : tk('StatOnTrack'),
    tagIcon:
      idleCount.value > 0
...
<style scoped>
.ds-entity-list-hero__tile--featured {
