    ...
    <template v-else>
      <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <AdminStatCard
          ...
          :trend="`+${overview?.tenant_count ?? 0} total`"
          trend-positive
        >
          <template #icon>
            <svg class="h-5 w-5 text-ds-fg-brand" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" aria-hidden="true">
        ...
        >
          <template #icon>
            <svg class="h-5 w-5 text-ds-success" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </template>
        </AdminStatCard>

        <AdminStatCard :label="$t('userTotal')" :value="overview?.user_count ?? 0">
          <template #icon>
            <svg class="h-5 w-5 text-ds-fg-brand" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" aria-hidden="true">
        ...
        <AdminStatCard :label="$t('adminInactiveTenants')" :value="inactiveCount">
          <template #icon>
            <svg class="h-5 w-5 text-ds-fg-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" aria-hidden="true">
      ...
      <section class="grid gap-6 lg:grid-cols-3">
        <div class="lg:col-span-2">
          <AdminOverviewChart
            :active-tenants="overview?.active_tenant_count ?? 0"
            :total-tenants="overview?.tenant_count ?? 1"
          />
        </div>
