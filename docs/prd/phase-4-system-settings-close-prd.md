...
| 用户数展示 | 角色卡片展示 `user_count` | 成员分配入口（`users/:id/roles`）可 Phase 4 Should |
...
#### 3.5.5 数据范围（Should，可与 RB-03 分迭代）

| Story ID | 作为 | 我要 | AC |
|----------|------|------|-----|
| RB-08 | Tenant Admin | 为角色配置 `data_scope`（`self` / `department` / `all`） | 保存后 `GET /api/rbac/my-permissions` 返回与角色一致；Dashboard/Leads 统计范围联动 |

> MVP 若未做 RB-08 UI，经理/销售仍使用 seed 策略；须在发布说明中标注，并在 Phase 5 组织与权限合并迭代。
...
| Should | 审计摘要导出能力 |
| Should | 角色级 `data_scope` 配置（RB-08） |
| Should | 用户分配角色（`POST /api/rbac/users/:id/roles`）在设置内入口 |
| Could | 健康度阈值告警与推荐动作；权限变更二次确认 Modal |
| Won't（MVP） | **组织架构树**（部门/岗位层级，对标截图左侧）；按品牌/代理商等细粒度数据权限 |
