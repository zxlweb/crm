...
**版本**：Phase 4 v0.4  
**日期**：2026-05-26  
**状态**：实现中（含成员/部门 v0.3、**权限会话与成员范围 v0.4** 增补）  
...
| G7 | 销售登录后残留 Demo 租户 cookie → 全站 403 | 登录/刷新 **强制 switch-tenant**；成员 API 按数据范围过滤 | 销售账号稳定看到本租户本部门数据 |
...
| 配置键 | 说明 |
|--------|------|
| `tenants.config.sales_quota` | 集团月度目标（租户管理员仪表盘 Gauge） |
| `tenants.config.department_quotas` | 对象：`{ "神龙云计算": { "amount", "currency", "period" }, ... }`；键须与 `user_tenants.department` **完全一致** |

演示 seed：`00021_xiaoxi_department_quotas.sql`；神龙事业部另含销售代表账号见 `00023_seed_xiaoxi_shenlong_sales_reps.sql`。
...
| MB-05 | Sales Manager | 在「成员」Tab 仅见 **本事业部** 成员 | `GET /api/rbac/members` 经 `filterMembersByScope`：`department` 时按 `user_tenants.department` 过滤；**非**全租户 10 人 |
| MB-06 | Tenant Admin | 成员 Tab 不阻塞于「基础配置」加载失败 | 设置页 `?tab=members` / `roles` 与「基础配置」Tab **独立 loading**；进入成员即可拉取列表 |

**成员列表说明文案**：

| 查看者 | 副标题 i18n |
|--------|-------------|
| 租户管理员 | `membersSectionDesc`：本租户全部成员 |
| 销售经理（`rbac:view` 无 `manage`） | `membersSectionDescDept`：仅显示本事业部「{department}」的成员 |

### 3.7 登录、租户与会话（权限生效前提 · Must）

> 权限变更与数据范围均依赖 **JWT 内租户 ID** 与 **Casbin 策略租户一致**；仅写 `crm.tenant_id` cookie 而不 `switch-tenant` 会导致 CRM/Dashboard **403、页面无数据**。

| Story ID | 作为 | 我要 | AC |
|----------|------|------|-----|
| AUTH-01 | 任意租户用户 | 登录后自动绑定正确租户 | 单租户用户：BE `Login` 直接 `issueTokensForTenant`；多租户：FE `finishSession` 调用 `POST /api/auth/switch-tenant` |
| AUTH-02 | 任意租户用户 | 刷新页面后权限与租户不错乱 | 插件 `01-rbac`：租户 cookie 变化时 **强制重拉** `my-permissions`；`02-tenant`：无效 cookie 时 `switchTenant` 到合法租户 |
| AUTH-03 | 销售经理 | 登录响应含 `department`、`current_role` | `user_tenants.department` 写入 `crm.department` cookie；工作台标题可展示事业部名 |
| AUTH-04 | 用户 | 退出后清空租户/权限缓存 | `logout` 清除 token、tenant、department、role、`rbac.permissions` |

**验收**：`shenlong@xiaoxi.com` 登录后 `crm.tenant_id` = 小西 UUID；`GET /api/dashboard/summary` 200；**不得**仍为 Demo 租户 `aaaaaaaa-…`。
...
5. `shenlong@xiaoxi.com`（神龙销售经理）→ `/settings?tab=members` 仅见神龙事业部成员（陈洋、赵云、孙丽等），不见灵狐/莫邪成员  
6. 销售账号退出后以 `shenlong@xiaoxi.com` 重登 → 工作台有 KPI/漏斗，无全站 403  
...
| 2026-05-26 | PM v0.4：§3.7 登录/租户/权限会话（AUTH-01–04）；MB-05/06 成员列表按 scope 过滤与 Tab 独立加载；`department_quotas` 配置说明 |
