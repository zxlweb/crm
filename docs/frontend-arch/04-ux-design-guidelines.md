...
EnterpriseFlow B2B CRM 前端用户体验与交互约定。视觉 Token 与主题实现见 [03-design-system.md](./03-design-system.md)。

| 文档 | 职责 |
|------|------|
| 本文档 | 体验原则、布局、交互、反馈、动效、无障碍 |
| [03-design-system.md](./03-design-system.md) | Token、主题、`Card*` / `Chart*` API |
| [05-component-scenarios.md](./05-component-scenarios.md) | **场景手册**（看板用 `CardMetric`，图表用 `Chart*`） |
| [01-directory-structure.md](./01-directory-structure.md) | 目录结构 |
| [02-ui-kit-modules.md](./02-ui-kit-modules.md) | Monorepo、双包、ui-kit 工程、bridge、排障 |
...
完整变量表见 [03-design-system.md](./03-design-system.md)。
...
### 5.5 看板 KPI · `CardMetric`（场景 `dashboard`）

> 场景登记：[05-component-scenarios.md](./05-component-scenarios.md) Part A · API：[03-design-system.md](./03-design-system.md) §5

看板、Admin、Dashboard 顶栏 KPI **统一** `<CardMetric />`（场景 `dashboard`），禁止手写卡片或图2式右上大图标布局。

#### 布局（必须）

```
┌────────────────────────────────────┐
│ (○ 圆标)  指标名 · text-ds-sm muted │
│           主数值 · text-ds-3xl bold │
│                                    │
│ 同比/环比标签          12% ↓/↑    │
└────────────────────────────────────┘
```
...
| 区域 | 规则 |
|------|------|
| 图标 | **左侧** 圆形 40px；`iconTone` 区分语义（info / calendar / accent / brand / neutral） |
| 主数值 | L4 层级；`tabular-nums`；千分位格式化 |
| 底栏 | `justify-between`：左对比文案（muted），右趋势 + 箭头 |
| 容器 | `.ds-card` + `rounded-ds-xl` + `p-ds-5` + `shadow-ds-sm` |

#### 趋势语义

| 方向 | 色 | 箭头 |
|------|-----|------|
| 增长/正向 | `text-ds-success`（`trend-direction="up"`） | ↑ |
| 下降/负向 | `text-ds-danger`（`trend-direction="down"`） | ↓ |
| 持平/无 | `text-ds-fg-muted`（`flat`） | 无 |

#### 栅格

- `grid gap-ds-4 sm:grid-cols-2 xl:grid-cols-4`
- 同行卡片使用 **不同** `iconTone`，避免四张完全相同紫色块

#### 禁止（图2 及变体）

- 图标在卡片 **右上角** 大方形容器
- 趋势与 label 挤在同一列、无底栏分隔
- 下降趋势仅用 muted 灰（必须用 danger 红）
- 在 `apps/web` 手写 KPI 卡（须用 `<CardMetric />`）

### 5.6 图表容器卡

- 外壳：`.ds-card` + `rounded-ds-2xl`
- 图表：`<ChartShell>`，标题区 `border-b border-ds-border-muted`
- Hover：可选轻微 `shadow-ds-md`（KPI 卡默认静态，不必 hover 抬升）

---

## 6. 数据可视化 UX

图表统一走 **`@crm/ui-kit`**（`<Chart*>` 组件）；实现位于 `packages/ui-kit`。业务页 **不要**调用 `useChartTheme` 或手写 ECharts option。
...
- [ ] 权限与租户边界符合 §9
- [ ] 组件归属符合 [02-ui-kit-modules.md](./02-ui-kit-modules.md)（设计系统 → ui-kit，业务 → apps/web）；`pnpm --filter @crm/web lint:layers` 通过
...
| `packages/ui-kit` — `UiThemeToggle` | 主题切换控件 |
| `packages/ui-kit` — `Chart*` | 图表族 |
